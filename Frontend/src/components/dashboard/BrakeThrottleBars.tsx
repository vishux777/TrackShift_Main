      ...
      <div className="flex flex-col items-center gap-3">
        <div className="relative w-20 h-52 bg-racing-panel/50 rounded-xl border border-racing-border/50 overflow-hidden backdrop-blur-sm">
          ...
          <motion.div
            ...
            initial={{ height: 0 }}
            animate={{ height: `${brake}%` }}
            transition={{ type: "spring", stiffness: 100, damping: 15 }}
          >
            <div className="absolute inset-0 bg-gradient-to-t from-primary via-racing-red-glow to-primary" />
            <div className="absolute inset-0 bg-gradient-to-t from-primary via-racing-red-glow to-primary blur-xl opacity-50" />
          </motion.div>
          ...
          {[0, 25, 50, 75, 100].map((mark) => (
            <div
              ...
              style={{ bottom: `${mark}%` }}
            >
              <span className="absolute -right-9 -translate-y-1/2 text-xs text-muted-foreground/60 font-mono">
                {mark}
              </span>
            </div>
          ))}
        ...
        <div className="text-3xl font-bold text-foreground font-mono tracking-tighter">{brake}<span className="text-lg text-muted-foreground">%</span></div>
      </div>
